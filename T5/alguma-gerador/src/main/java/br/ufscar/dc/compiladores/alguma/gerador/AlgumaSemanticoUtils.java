...
import br.ufscar.dc.compiladores.alguma.gerador.TabelaDeSimbolos.EntradaTabelaDeSimbolos;
...
public class AlgumaSemanticoUtils {
    ...
    public static boolean ehTipoNumeral(TipoAlguma t1, TipoAlguma t2) {
        return (t1 == TipoAlguma.INTEIRO && t2 == TipoAlguma.REAL) || (t1 == TipoAlguma.REAL && t2 == TipoAlguma.INTEIRO);
    }

    public static boolean ehTipoInteiroEmReal(TipoAlguma t1, TipoAlguma t2) {
        return t1 == TipoAlguma.REAL && t2 == TipoAlguma.INTEIRO;
    }
    ...
    public static InfoIdentificador verificarIdentificador(Escopos pilhaDeTabelas, AlgumaParser.IdentificadorContext ctx) {
        ...
        if (acessoVetor) {
            ...
            for (var exp : ctx.dimensao().exp_aritmetica()) {
                if (verificarTipo(pilhaDeTabelas, exp) != TipoAlguma.INTEIRO)
    ...
    public static TipoAlguma verificarTipo(Escopos pilhaDeTabelas, AlgumaParser.ExpressaoContext ctx) {
        ...
        for (int i = 1; i < ctx.termo_logico().size(); i++) {
            ...
            if (tipoResultante != TipoAlguma.LOGICO || proximoTipo != TipoAlguma.LOGICO)
                return TipoAlguma.INVALIDO;
        }
    ...
    public static TipoAlguma verificarTipo(Escopos pilhaDeTabelas, AlgumaParser.Exp_relacionalContext ctx) {
        ...
        if (tipo1 == TipoAlguma.INVALIDO || tipo2 == TipoAlguma.INVALIDO)
            return TipoAlguma.INVALIDO;
    ...
    public static TipoAlguma verificarTipo(Escopos pilhaDeTabelas, AlgumaParser.Exp_aritmeticaContext ctx) {
        ...
        for (int i = 1; i < ctx.termo().size(); i++) {
            ...
            if (proximoTipo == TipoAlguma.INVALIDO)
                return TipoAlguma.INVALIDO;
            if (ehTipoNumeral(tipoResultante, proximoTipo))
                tipoResultante = TipoAlguma.REAL;
            else if (tipoResultante == TipoAlguma.LITERAL && proximoTipo == TipoAlguma.LITERAL)
                tipoResultante = TipoAlguma.LITERAL;
    ...
    public static void verificarParametros(Escopos pilhaDeTabelas, EntradaTabelaDeSimbolos subRotina,
            List<AlgumaParser.ExpressaoContext> args, Token tokenChamada) {
